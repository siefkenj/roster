...
class ActiveUserService
    ...
    def self.active_user(request)
        ...
        if Rails.application.config.allow_basic_auth &&
            ...
            user = User.find_or_create_by(utorid: utorid)
            user.touch(:last_seen)
            return logged_user(user, 'Basic Auth')
        end
        ...
        if (Rails.application.config.respond_to? :active_user_override) &&
            ...
            user =
                User.find_by(id: Rails.application.config.active_user_id) ||
                    User.new(utorid: 'defaultactive')
            user.save!
            user.touch(:last_seen)

            return logged_user(user, 'Debug ActiveUser Override')
        end
        ...
        if request.env['HTTP_X_FORWARDED_USER']
            ...
            user = User.find_or_create_by(utorid: utorid)
            user.touch(:last_seen)
            return logged_user(user, 'Shibboleth')
        end
